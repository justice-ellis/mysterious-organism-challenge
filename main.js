...
const returnRandBase = () => {
  ...
  // Return a random single stand of DNA containing 15 bases
  const mockUpStrand = () => {
    const newStrand = []
    for (let i = 0; i < 15; i++) {
  ...
  const pAequorFactory = (num, bases) => {
    ...
    return {
      ...
      compareDNA(pAequor) {   // Compare Current and past values of pAequor
        numberCommonality = 0;
  ...
  // Create 30 pAequor that are likely to survive
  const survivors = [];
  ...
  // Call the function survivors
  console.log(survivors);
  


 //---------------by: Ellis-Justice-Nana----------!
